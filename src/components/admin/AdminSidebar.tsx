...
export default function AdminSidebar({ isCollapsed = false, onToggleCollapse }: AdminSidebarProps) {
  ...
  return (
    <aside className="relative flex h-full flex-col gap-6 rounded-3xl border border-white/10 bg-[linear-gradient(160deg,rgba(18,18,18,0.95),rgba(6,6,6,0.88))] p-6 text-sm shadow-[0_45px_90px_-60px_rgba(0,0,0,0.85)] backdrop-blur xl:sticky xl:top-10 transition-all duration-300">
      ...
      {/* Header with Logo and Mobile/Desktop Toggle */}
      <div className="relative flex items-center justify-between">
        ...
        <button
          ...
          className="hidden xl:flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-black/60 text-white transition hover:border-[rgba(201,161,77,0.45)] hover:text-main"
        ...
        <button
          ...
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-black/60 text-white transition hover:border-[rgba(201,161,77,0.45)] hover:text-main xl:hidden"
          aria-label="Toggle Navigation Menu"
        >
          {isOpen ? <PiX className="h-6 w-6" /> : <PiList className="h-6 w-6" />}
        </button>
      </div>

      {/* Collapsible Area */}
      <div className={`${isOpen ? 'flex' : 'hidden'} flex-col gap-6 xl:flex`}>
